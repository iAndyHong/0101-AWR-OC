...
//|                                                                   |
//| 引用方式：#include "../Libs/GridsCore/CGridsCore_v2.2.mqh"        |
//+------------------------------------------------------------------+
...
#include "../EACore/CEACore_v2.2.mqh"
...
struct GridsCoreConfig
  {
   ...
   color             stBullColor;
    ...
    double            heikenThreshold;    // Heiken Ashi 趨勢確認閾值（已不使用，保留參數為相容性）
...
private:
   ...
   int               m_superTrendPrevDirection;
    ...
    int               m_heikenConsecutive;  // 趨勢連續計數
...
int CGridsCore::CalculateHeiken()
  {
   ...
   // Heiken Ashi Smoothed 演算法
   // 步驟 1: 使用 MA 對原始價格進行平滑處理
   double maOpen  = iMA(m_config.symbol, tf, m_config.heikenMaPeriod, 0, m_config.heikenMaMethod, PRICE_OPEN, 0);
   double maClose = iMA(m_config.symbol, tf, m_config.heikenMaPeriod, 0, m_config.heikenMaMethod, PRICE_CLOSE, 0);
   double maLow   = iMA(m_config.symbol, tf, m_config.heikenMaPeriod, 0, m_config.heikenMaMethod, PRICE_LOW, 0);
   double maHigh  = iMA(m_config.symbol, tf, m_config.heikenMaPeriod, 0, m_config.heikenMaMethod, PRICE_HIGH, 0);

   // 步驟 2: 計算 Heiken Ashi
   // HA_Open = (前一個 HA_Close + 前一個 HA_Open) / 2.0
   // HA_Close = (maOpen + maHigh + maLow + maClose) / 4.0
   ...
   int newDirection = 0;
   if(m_heikenClose > m_heikenOpen) newDirection = 1;  // 紅色漲
   else if(m_heikenClose < m_heikenOpen) newDirection = -1;  // 綠色跌
   else newDirection = 0;  // 十字星

   // 趨勢連續性計數
   if(newDirection != 0 && newDirection == m_heikenDirection)
     {
      m_heikenConsecutive++;
     }
   else
     {
      m_heikenConsecutive = (newDirection != 0) ? 1 : 0;
     }
   m_heikenDirection = newDirection;

   // 判斷趨勢是否反轉
   m_heikenReversed = (m_heikenDirection != m_heikenPrevDirection && m_heikenPrevDirection != 0);
   if(m_heikenReversed) m_tradedThisSignal = false;
   ...
   if(m_config.heikenSignalMode == SIGNAL_MODE_REVERSAL)
     {
      // 反轉模式：只在趨勢反轉時發出信號
      if(m_heikenReversed && !m_tradedThisSignal)
        {
         return (m_heikenDirection == 1) ? SIGNAL_BUY : (m_heikenDirection == -1 ? SIGNAL_SELL : SIGNAL_NEUTRAL);
        }
      return SIGNAL_NEUTRAL;
     }
   else if(m_config.heikenSignalMode == SIGNAL_MODE_TREND)
     {
      // 趨勢模式：只要變色就發出信號（Heiken Ashi Smoothed 本身已有平滑）
...
bool CGridsCore::AllowFirstOrder()
  {
   ...
   if(m_config.filterMode == FILTER_HeikenAshi && m_config.heikenSignalMode == SIGNAL_MODE_REVERSAL)
     {
      return (m_heikenReversed && !m_tradedThisSignal);
     }
   return true;
  }
...
bool CGridsCore::AllowAveraging(int dir)
  {
   ...
   if(m_config.filterMode == FILTER_HeikenAshi && m_config.heikenAveragingMode == AVERAGING_TREND)
     {
      int s = GetTrendSignal();
      if(dir == OP_BUY) return (s == SIGNAL_BUY || s == SIGNAL_NEUTRAL);
      if(dir == OP_SELL) return (s == SIGNAL_SELL || s == SIGNAL_NEUTRAL);
...
bool CGridsCore::OpenGridOrder(int type, double lots)
  {
   ...
   if(m_config.maxSpread > 0 && spread > m_config.maxSpread)
     {
      WriteLog("[GC] OpenGridOrder Failed: Spread too high (" + DoubleToString(spread, 0) + " > " + DoubleToString(m_config.maxSpread, 0) + ")");
      return false;
     }
   ...
   if(t < 0)
     {
      ...
      WriteLog("[GC] OrderSend Failed: Error " + IntegerToString(err) + " | Type: " + (type==OP_BUY?"BUY":"SELL") + " | Lots: " + DoubleToString(lots, 2) + " | Price: " + DoubleToString(p, m_digits));
      return false;
     }
   return true;
  }
...
void CGridsCore::Execute(const OrderStats &stats)
  {
   if(!m_initialized) return;
   if(stats.buyCount == 0 && m_buyGridLevel > 0) { m_buyGridLevel = 0; m_buyBasePrice = 0; m_lastBuyDistLevel = -1; }
   if(stats.sellCount == 0 && m_sellGridLevel > 0) { m_sellGridLevel = 0; m_sellBasePrice = 0; m_lastSellDistLevel = -1; }
   ...
   // Heiken Ashi 反轉重置邏輯已移除，改為對沖模式（直到 TP 全平）
      ...
      if(m_buyGridLevel == 0 && stats.buyCount == 0 && (sig == SIGNAL_BUY || isSimp) && allow1 && ok)
        {
         ...
         if(OpenGridOrder(OP_BUY, l)) { m_buyGridLevel = 1; m_buyBasePrice = cp; m_lastBuyBarTime = curBT; WriteLog("[GC] ENTRY BUY | Lots: " + DoubleToString(l, 2) + " | Price: " + DoubleToString(cp, m_digits) + " | Level 1"); }
        }
      else if(m_buyGridLevel > 0 && m_buyGridLevel < m_config.maxGridLevels && ok)
        {
         if(AllowAveraging(OP_BUY))
           {
            double d = GetCumulativeDistance(OP_BUY, m_buyGridLevel);
            double tp = (m_config.gridMode == GRID_MODE_COUNTER) ? (m_buyBasePrice - d) : (m_buyBasePrice + d);
            if((m_config.gridMode == GRID_MODE_COUNTER && cp <= tp) || (m_config.gridMode == GRID_MODE_TREND && cp >= tp))
              {
               ...
               if(OpenGridOrder(OP_BUY, l)) { m_buyGridLevel++; m_lastBuyBarTime = curBT; WriteLog("[GC] ENTRY BUY | Lots: " + DoubleToString(l, 2) + " | Price: " + DoubleToString(cp, m_digits) + " | Level " + IntegerToString(m_buyGridLevel)); }
              }
           }
        }
     }
      ...
      if(m_sellGridLevel == 0 && stats.sellCount == 0 && (sig == SIGNAL_SELL || isSimp) && allow1 && ok)
        {
         ...
         double l = CalculateLots(1);
         if(OpenGridOrder(OP_SELL, l)) { m_sellGridLevel = 1; m_sellBasePrice = cp; m_lastSellBarTime = curBT; WriteLog("[GC] ENTRY SELL | Lots: " + DoubleToString(l, 2) + " | Price: " + DoubleToString(cp, m_digits) + " | Level 1"); }
        }
      else if(m_sellGridLevel > 0 && m_sellGridLevel < m_config.maxGridLevels && ok)
        {
         if(AllowAveraging(OP_SELL))
           {
            double d = GetCumulativeDistance(OP_SELL, m_sellGridLevel);
            double tp = (m_config.gridMode == GRID_MODE_COUNTER) ? (m_sellBasePrice + d) : (m_sellBasePrice - d);
            if((m_config.gridMode == GRID_MODE_COUNTER && cp >= tp) || (m_config.gridMode == GRID_MODE_TREND && cp <= tp))
              {
               double l = CalculateLots(m_sellGridLevel + 1);
               if(OpenGridOrder(OP_SELL, l)) { m_sellGridLevel++; m_lastSellBarTime = curBT; WriteLog("[GC] ENTRY SELL | Lots: " + DoubleToString(l, 2) + " | Price: " + DoubleToString(cp, m_digits) + " | Level " + IntegerToString(m_sellGridLevel)); }
              }
           }
        }
     }
   ...
   if(m_config.takeProfit > 0 && p >= m_config.takeProfit)
     {
      ...
      WriteLog("[GC] EXIT ALL | Profit: " + DoubleToString(cp, 2) + " | Price: " + DoubleToString(MarketInfo(m_config.symbol, MODE_BID), m_digits) + " | TakeProfit");
      ...
      if(OrderSelect(i, SELECT_BY_POS) && OrderMagicNumber() == m_config.magicNumber && OrderSymbol() == m_config.symbol)
        {
         if(!OrderClose(OrderTicket(), OrderLots(), (OrderType() == OP_BUY ? Bid : Ask), m_config.slippage))
           {
            WriteLog("[GC] Failed to close order " + IntegerToString(OrderTicket()));
           }
        }
     }
   return p;
...
void CGridsCore::ResetBaskets() { m_buyGridLevel = 0; m_sellGridLevel = 0; m_buyBasePrice = 0; m_sellBasePrice = 0; m_lastBuyDistLevel = -1; m_lastSellDistLevel = -1; m_lastBuyBarTime = 0; m_lastSellBarTime = 0; m_tradedThisSignal = false; }
