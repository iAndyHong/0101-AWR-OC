...
//| 引入核心模組 (v2.2 版本)                                         |
//+------------------------------------------------------------------+
#include "../Libs/EACore/CEACore_v2.2.mqh"
#include "../Libs/GridsCore/CGridsCore_v2.2.mqh"
...
input int      HK_MaPeriod               = 6;                    // 平滑週期
input ENUM_MA_METHOD HK_MaMethod         = MODE_SMMA;            // 平滑方法
...
// Heiken Ashi 參數
   ...
   if(!g_gridsCore.Init(config))
     {
      Print("[Grids 2.2] CGridsCore 初始化失敗");
      return INIT_FAILED;
     }
...
// 設定回調函數
   ...
   Print("[Grids 2.2] EA 初始化完成 - 組別: ", PG_GroupID, ", Magic: ", PG_MagicNumber);
...
void OnDeinit(const int reason)
  {
   ...
   Print("[Grids 2.2] EA 已卸載，原因: ", reason);
  }
