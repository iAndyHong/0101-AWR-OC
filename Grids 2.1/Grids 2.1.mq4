...
//| 引入核心模組 (v2.1 版本)                                         |
//+------------------------------------------------------------------+
#include "../Libs/EACore/CEACore_v2.1.mqh"
#include "../Libs/GridsCore/CGridsCore_v2.1.mqh"
...
// 獨立縮放設定（0 = 不縮放）
   ...
   config.stAveragingMode = ST_AveragingMode;
    ...
    if(!g_gridsCore.Init(config))
     ...
     {
      Print("[Grids 2.1] CGridsCore 初始化失敗");
      return INIT_FAILED;
     }
...
// 設定回調函數
   ...
   Print("[Grids 2.1] EA 初始化完成 - 組別: ", PG_GroupID, ", Magic: ", PG_MagicNumber);
...
void OnDeinit(const int reason)
  {
   ...
   Print("[Grids 2.1] EA 已卸載，原因: ", reason);
  }
