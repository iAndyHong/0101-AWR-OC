...
//|
//| 引用方式：#include "../Libs/GridsCore/CGridsCore_v2.4.mqh"
//+------------------------------------------------------------------+

#ifndef CGRIDSCORE_V24_MQH
#define CGRIDSCORE_V24_MQH
...
// 引用 CEACore 的 OrderStats 結構定義
#include "../EACore/CEACore_v2.4.mqh"
...
int CGridsCore::GetTrendSignal()
  {
   ...
   // 優化：在回測模式下，每根 K 線只計算一次信號
   // 如果不是新 K 線，直接返回快取的信號，避免重複執行複雜的 MA 計算
   if(IsTesting() && !IsNewBar(tf) && m_lastTrendCalcTime > 0) return m_cachedTrendSignal;
   
   // 非回測模式下，則根據時間間隔進行更新
   if(!IsTesting() && (TimeCurrent() - m_lastTrendCalcTime < 1) && m_lastTrendCalcTime > 0) return m_cachedTrendSignal;
...
void CGridsCore::Execute(const OrderStats &stats)
  {
   ...
   // 使用傳入的統計數據，避免重複掃描
   int currentBuyCount = stats.buyCount;
   int currentSellCount = stats.sellCount;
   ...
   if(m_config.filterMode == FILTER_HeikenAshi && m_heikenReversed)
     {
      double currentTotalProfit = stats.profit; // 直接使用 stats 盈虧
