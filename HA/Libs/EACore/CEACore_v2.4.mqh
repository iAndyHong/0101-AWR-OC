...
//|
//| 引用方式：#include "../Libs/EACore/CEACore_v2.4.mqh"
//+------------------------------------------------------------------+

#ifndef CEACORE_V24_MQH
#define CEACORE_V24_MQH
...
#include "../UI/CChartPanelCanvas_v2.4.mqh"
...
//| 取得訂單統計數據
OrderStats CEACore::GetOrderStats()
{
   UpdateOrderCache();
   return m_stats;
}
...
void CEACore::UpdatePanel(bool forceUpdate = false)
{
   ...
   if(m_chartPanel.UpdateWithStats(m_stats, forceUpdate))
   {
      m_lastUIPanelUpdate = TimeCurrent();
   }
}
...
{
   string msg = "[EA] " + action + " | Lots: " + DoubleToString(lots, 2) + " | Price: " + DoubleToString(price, m_cachedDigits) + " | " + remark;
   WriteLog(msg);
}
